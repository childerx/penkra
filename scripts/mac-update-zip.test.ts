...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Penkra.app"), [
      "Penkra.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Penkra.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Penkra.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Penkra.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Penkra.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Penkra.app/Contents/Info.plist",
        "Penkra.app/Contents/Info.plist",
        "Penkra.app/Contents/MacOS/Penkra",
      ]),
      "Penkra.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Penkra-0.1.5-arm64.dmg",
        "Penkra-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Penkra-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Penkra-0.1.5-arm64.zip", "Penkra-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Penkra-0.1.5-arm64.dmg"]),
...
files:
  - url: Penkra-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Penkra-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Penkra-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Penkra-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Penkra-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Penkra-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Penkra-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Penkra-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Penkra-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Penkra-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
