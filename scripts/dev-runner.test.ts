...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit PENKRA_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "PENKRA_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid PENKRA_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("boolean precedence", () => {
    ...
    it.effect("rejects invalid boolean environment values", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          readDevRunnerBooleanEnvironment({ PENKRA_LOG_WS_EVENTS: "sometimes" }),
        );
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults PENKRA_HOME to ~/.penkra when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PENKRA_HOME, resolve(homedir(), ".penkra"));
        assert.equal(env.PENKRA_HOST, "127.0.0.1");
    ...
    it.effect("normalizes bracketed IPv6 hosts for listen and client URL syntax", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PENKRA_HOST, "::1");
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PENKRA_HOME, resolve("/tmp/custom-penkra"));
        assert.equal(env.PENKRA_PORT, "4222");
        assert.equal(env.PENKRA_NO_BROWSER, "1");
        assert.equal(env.PENKRA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.PENKRA_LOG_WS_EVENTS, "1");
        assert.equal(env.PENKRA_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            PENKRA_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          penkraHome: undefined,
        ...
        assert.equal(env.PENKRA_MODE, "web");
        assert.equal(env.PENKRA_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PENKRA_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom penkraHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.PENKRA_HOME, resolve("/tmp/my-penkra"));
        assert.equal(env.PENKRA_HOME, resolve("/tmp/my-penkra"));
        assert.equal(env.PENKRA_HOME, resolve("/tmp/my-penkra"));
      }),
    );
