...
You are GPT-5.5 (xhigh) in the Penkra monorepo at `/Users/emanueledipietro/Developer/penkra`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `PENKRA_HOME` (dev: `./.penkra/electron-dev`; prod: `~/.penkra` or
  `~/Documents/Penkra`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `PENKRA_HOME/PENKRA_HOME/PENKRA_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.penkra/electron-dev/dev/state.sqlite` (dev) and `~/.penkra/userdata/state.sqlite`
     (prod) where relevant.
