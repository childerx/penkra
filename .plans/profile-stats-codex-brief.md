...
You are GPT-5.5 (xhigh) working in the Penkra monorepo at `/Users/emanueledipietro/Developer/penkra`.
...
- BRANCH dev DB (what the dev instance uses): `./.penkra/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.penkra/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@penkra/contracts --filter=@penkra/cli` must pass. DO NOT run the web
   (`@penkra/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
