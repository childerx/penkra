...
- Repository: `https://github.com/Emanuele-web04/penkra`
- Release tag: `v0.5.5`
...
- An isolated web instance runs with `PENKRA_PORT_OFFSET=3158`, server port `58090`, web port
  `8891`, and state under `.penkra-penkra-phase0`.
...
- Skill references use `$name`. Upstream already parses `$name` as a skill token in
  ...
  Penkra skills must remain server-backed and load through `penkra skill load`, not Penkra's local
...
- `apps/server/src/main.ts` already resolves `PENKRA_HOME`/`--home-dir` into `ServerConfig.baseDir`.
  Desktop bootstrap should set `PENKRA_HOME`; no alternate storage root is needed.
...
- The project row is rendered by `renderProjectItem` in
  ...
  signal and source it from the Penkra store; do not overload Penkra's thread status glyph.
