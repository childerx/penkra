...
describe("generated App help", () => {
  it("combines package instructions with direct App-root commands", () => {
    const help = generateAppHelp({ manifest, instructions: "Create issues only after confirming the project." });
  ...
  it("renders operation usage and both declared schemas", () => {
    const help = generateAppHelp({ manifest, instructions: "Follow workspace conventions.", operation: "issues.create" });
    expect(help).toContain("--input '<json>' [--tab-id <tab-id>]");
