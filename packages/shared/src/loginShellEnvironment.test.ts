...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "penkra-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Penkra home.
    const env = { PENKRA_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Penkra home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { PENKRA_HOME: "/tmp/penkra-home" } })).toBe(
      Path.join("/tmp/penkra-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".penkra", "cache", "login-shell-environment.json"),
    );
  });
});
