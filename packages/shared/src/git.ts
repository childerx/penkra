 ...
 */
export const WORKTREE_BRANCH_PREFIX = "penkra";
...
const PENKRA_BRANCH_FALLBACK = "update";
...
export function buildPenkraBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || PENKRA_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniquePenkraBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildPenkraBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
