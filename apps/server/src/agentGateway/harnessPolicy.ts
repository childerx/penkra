...
export const PENKRA_HARNESS_POLICY_VERSION = "2026-08-04.5";
...
export function renderPenkraHarnessPolicy(capabilities: PenkraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use `penkra_exec_command` for every Penkra operation: context and capabilities; Projects and Threads; Apps and App operations; App tabs and observation; resource routing; diagnostics; and any native Connectors registered by this Penkra build. It accepts exactly one registered Penkra command. It is a first-class host tool, not a shell, wildcard tool name, terminal program, or namespace for other provider tools.",
        "Use the provider's ordinary shell or command-execution tool for operating-system commands and native executables. Providers name that tool differently; do not assume it is called `exec_command`. Native programs remain outside Penkra's command registry, and an App or Connector slug never shadows PATH.",
        ...
        "A Penkra Connector is trusted host integration code for a specifically supported native application detected on this computer. It may mirror structured capabilities from that application's SDK, IPC service, CLI, or MCP implementation, but users and agents never configure or call an MCP server for it. A Connector is available only when the current `penkra --help` catalog exposes connector discovery and that discovery reports it available; never invent a Connector from an installed executable or provider integration.",
        ...
        "A Skill supplies instructions, never capabilities. Loading a Skill does not install or authorize any App, Connector, MCP server, plugin, executable, browser, or tool that its text mentions. Before a capability-dependent step, verify availability through the literal provider tool catalog, `penkra --help` plus the relevant Penkra discovery command, or the provider's ordinary shell for a native executable. If the required capability is absent, continue only with independent valid steps and report the missing capability; never silently substitute a different category.",
        "Penkra disables external, user, project, and provider-bundled MCP servers in managed provider sessions. The private Penkra Agent Gateway is the sole MCP transport Penkra injects and is invisible product infrastructure. Provider connector catalogs and provider-controlled in-app browsers are unavailable. A literal provider tool may still expose approved local artifact work, native Computer Use, or control of the external Chrome application; use such a tool only for its described purpose and never present it as a Penkra App or Connector.",
