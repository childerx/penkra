...
export function renderPenkraHarnessPolicy(capabilities: PenkraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Start Penkra work with `penkra --help` when the relevant hierarchy is unknown. Use nested `--help` before an unfamiliar command; help includes the complete validated input contract. Penkra commands reject shell programs, pipes, redirects, substitutions, environment expansion, and command chaining.",
