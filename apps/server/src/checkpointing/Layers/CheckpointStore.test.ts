...
describe("CheckpointStoreLive", () => {
  ...
  it("deduplicates concurrent captures for the same checkpoint ref", async () => {
    ...
    await runtime.runPromise(
      Effect.gen(function* () {
        ...
        const input = {
          cwd: "/repo",
          checkpointRef: CheckpointRef.makeUnsafe("refs/penkra-checkpoints/thread/message"),
        };
  ...
  it("skips the capture when skipIfExists is set and the ref already exists", async () => {
    const existingRef = "refs/penkra-checkpoints/thread/existing";
    const missingRef = "refs/penkra-checkpoints/thread/missing";
    const execute = vi.fn<GitCoreShape["execute"]>((input) => {
      const args = input.args.join(" ");
  ...
  it("restores the worktree patch when resetting the index fails during file undo", async () => {
    const fromRef = CheckpointRef.makeUnsafe("refs/penkra-checkpoints/thread/turn/start");
    const toRef = CheckpointRef.makeUnsafe("refs/penkra-checkpoints/thread/turn/end");
  ...
  it("fails when a checkpoint ref cannot be deleted", async () => {
    const lockedRef = CheckpointRef.makeUnsafe("refs/penkra/checkpoints/thread/turn/locked");
    const deletableRef = CheckpointRef.makeUnsafe("refs/penkra/checkpoints/thread/turn/ok");
    const execute = vi.fn<GitCoreShape["execute"]>((input) => {
      const args = input.args.join(" ");
  ...
  it("tolerates deleting checkpoint refs that are already absent", async () => {
    ...
    const missingRef = CheckpointRef.makeUnsafe("refs/penkra/checkpoints/thread/turn/gone");
