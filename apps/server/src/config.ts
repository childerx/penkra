...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "PENKRA_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://penkra.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Penkra through PENKRA_PUBLIC_URL/--public-url without PENKRA_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Penkra to non-loopback host ${config.host ?? "<unspecified>"} without PENKRA_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with PENKRA_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with PENKRA_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.PENKRA_STATIC_DIR?.trim();
  if (snapshotDir) {
