...
export function remoteAccessPolicyError(
  ...
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Penkra through SYNARA_PUBLIC_URL/--public-url without SYNARA_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Penkra to non-loopback host ${config.host ?? "<unspecified>"} without SYNARA_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
