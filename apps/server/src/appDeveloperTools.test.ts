...
describe("App developer packaging", () => {
  it("creates reproducible archives and complete submission evidence", async () => {
    ...
    expect(one).toEqual(expect.objectContaining({
      ...
      permissions: [{ permission: "network-fetch", required: false, rationale: "Sync documents" }],
    }));
  ...
  it("rejects missing manifest references before writing an archive", async () => {
    ...
    await expect(packageAppDirectory({ directory: root, output: join(root, "..", "bad.penkra") }))
      .rejects.toThrow("Manifest reference is missing");
  });

  it("rejects symlinks and output paths inside the package root", async () => {
    const root = await fixture();
    await expect(packageAppDirectory({ directory: root, output: join(root, "app.penkra") }))
      .rejects.toThrow("outside the packaged directory");
    await rm(join(root, "app.html"));
    await import("node:fs/promises").then(({ symlink }) => symlink("README.md", join(root, "app.html")));
    await expect(packageAppDirectory({ directory: root, output: join(root, "..", "bad.penkra") }))
...
async function fixture(): Promise<string> {
  ...
  roots.push(root, join(root, "..", "first.penkra"), join(root, "..", "second.penkra"), join(root, "..", "bad.penkra"));
  ...
  await writeFile(join(root, "penkra-app.json"), JSON.stringify({
    manifestVersion: 1,
