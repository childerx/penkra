...
interface DroidSessionContext {
  ...
  // live updates keep draining), which means sendTurn can route to it mid-startup;
...
export function extractDroidApproveSpecPlanMarkdown(
  ...
  if (toolCall.title?.trim().toLowerCase() !== "approve spec") return undefined;
  const rawInput = toolCall.data.rawInput;
  if (typeof rawInput !== "object" || rawInput === null || !("plan" in rawInput)) return undefined;
  ...
  return Effect.gen(function* () {
    ...
    const startSession: DroidAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Config RPCs run after the consumer fork so live updates emitted while
          // they are in flight keep draining. The session is already registered and
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        // it must fail here instead of emitting lifecycle events for a dead session.
        if (ctx.stopped) {
        ...
        const promptText = appendFileAttachmentsPromptBlock({
          text: appendProviderReferencesPromptBlock({
            text: input.input?.trim() || undefined,
        ...
        // A stop can land while the startup gate or attachment reads above were
        ...
        const runPrompt = Effect.suspend(() =>
          // interruptTurn during the pre-prompt waits (startup configuration, attachment
    ...
    const interruptTurn: DroidAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
        ...
        // (it may be gated on startup configuration); flag it so startDroidTurn aborts
          ...
          Effect.gen(function* () {
            if (!(yield* runtime.supportsSessionFork)) {
              return yield* new ProviderAdapterValidationError({
                ...
                operation: "forkThread",
                issue:
                  "This Droid ACP version does not advertise session/fork; Penkra will rebuild the fork from its retained transcript.",
              });
            }
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Penkra Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Penkra Command Discovery",
          });
          yield* runtime.start();
