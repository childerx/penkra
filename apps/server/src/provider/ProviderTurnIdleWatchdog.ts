/**
 * ProviderTurnIdleWatchdog - idle-progress backstop for provider turns.
 *
 * provider runtimes drive a turn by issuing a single `session/prompt`
 ...
 * turns — any inbound provider activity resets it, and it pauses entirely while the
 * turn is legitimately blocked on a human approval.
 *
 * @module ProviderTurnIdleWatchdog
 */
import { Effect, Fiber, Scope } from "effect";

export interface ProviderTurnIdleWatchdogParams {
  /** How long the turn may go without any inbound provider activity before it is force-failed. */
  readonly idleTimeoutMs: number;
  /** Optional live override for work whose liveness is known outside the parent provider stream. */
...
export type ProviderTurnIdleTickDecision = "stop" | "touch" | "timeout" | "continue";
 ...
 */
export function evaluateProviderTurnIdleTick(input: {
  readonly isTurnActive: boolean;
  readonly isAwaitingHuman: boolean;
  readonly idleMs: number;
  readonly idleTimeoutMs: number;
}): ProviderTurnIdleTickDecision {
 ...
 */
export function resolveProviderTurnIdleTimeoutMs(input: {
 ...
 */
export const forkProviderTurnIdleWatchdog = (
  params: ProviderTurnIdleWatchdogParams,
