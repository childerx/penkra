import type { GitBranch } from "@penkra/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.penkra/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.penkra/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .penkra worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.penkra/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/penkra-pi",
        existingAssociatedWorktreeBranch: "penkra/pi",
        existingAssociatedWorktreeRef: "penkra/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/penkra-pi",
      associatedWorktreeBranch: "penkra/pi",
      associatedWorktreeRef: "penkra/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.penkra/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.penkra/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.penkra/worktrees/feature-b",
      nextWorktreePath: "/repo/.penkra/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.penkra/worktrees/feature-a",
      nextWorktreePath: "/repo/.penkra/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
