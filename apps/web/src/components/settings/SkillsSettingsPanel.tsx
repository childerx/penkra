      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Penkra could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Penkra skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
                    ...
                    <span className="flex min-w-0 flex-col gap-1">
                      ...
                      {group.sources.map((source) => (
                        <code
                          ...
                          className="truncate text-[length:var(--app-font-size-ui-sm,11px)] text-muted-foreground"
                        >
                          {source.skill.path}
                        </code>
                      ))}
