...
function PastedTextCardAction({ children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      type="button"
      className="-ml-px inline-flex w-fit items-center gap-0.5 text-[length:var(--app-font-size-ui-sm,11px)] text-muted-foreground underline decoration-muted-foreground/40 underline-offset-2 transition-colors hover:text-foreground hover:decoration-foreground/60 focus-visible:outline-none"
      {...props}
    >
      {children}
    </button>
...
export function UserMessagePastedTextCard({ text, metrics }: UserMessagePastedTextCardProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      ...
      />
      {expanded ? (
        <pre className="max-h-80 w-full max-w-full overflow-auto rounded-md border border-[color:var(--color-border-light)] bg-[var(--color-background-elevated-secondary)] p-2 font-mono text-[length:var(--app-font-size-ui-sm,11px)] leading-snug whitespace-pre-wrap break-words text-foreground">
          {text}
        </pre>
      ) : null}
    </div>
