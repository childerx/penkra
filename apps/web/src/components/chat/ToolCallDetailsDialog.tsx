...
export function ToolCallDetailsContent({
  ...
  if (!details && !activity) {
    return (
      <div className="rounded-lg border border-border/45 bg-background/60 px-3 py-2 text-[length:calc(var(--app-font-size-base,12px)*1.1667)] text-muted-foreground">
        ...
        <ToolDetailSection title="Files">
          <div className="flex flex-wrap gap-1.5">
            {details.files.map((file) => (
              <span
                key={file}
                className="max-w-full rounded-md border border-border/45 bg-background/70 px-2 py-1 font-chat-code text-[length:var(--app-font-size-ui-sm,11px)] text-foreground/82"
                title={file}
              >
                {file}
              </span>
        ...
        <ToolDetailSection title="Edits">
          <div className="space-y-3">
            {details.edits.map((edit, index) => (
              ...
              >
                {edit.path ? (
                  <div className="border-b border-border/45 px-3 py-2 font-chat-code text-[length:var(--app-font-size-ui-sm,11px)] text-muted-foreground/72">
                    {edit.path}
                  </div>
                ) : null}
...
function LiveActivityMetadata({
  ...
  return (
    <ToolDetailSection title="Activity">
      <dl className="grid grid-cols-[max-content_minmax(0,1fr)] gap-x-3 gap-y-1.5 rounded-lg border border-border/45 bg-background/60 px-3 py-2.5 text-[length:var(--app-font-size-ui-sm,11px)]">
...
function ToolOutputMetadata({ output }: { output: WorkLogToolOutputDetails }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-[length:var(--app-font-size-ui-sm,11px)] text-muted-foreground/68">
...
function DiffCodeBlock({ children }: { children: string }) {
  ...
  return (
    <pre className="max-h-[min(52vh,34rem)] overflow-auto rounded-lg border border-border/45 bg-background/70 px-0 py-2 font-chat-code text-[length:var(--app-font-size-ui-sm,11px)] leading-relaxed">
