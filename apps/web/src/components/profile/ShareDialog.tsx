...
export function ShareDialog({
  ...
  const handleSave = () => {
    ...
    return renderNodeToPngBlob(node, CARD_EXPORT_SIZE)
      .then((blob) => {
        if (blob) {
          downloadBlob(blob, `penkra-stats-${stats.timezone.today}.png`);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup className="sm:max-w-[560px]">
        ...
        <div className="mt-5 flex flex-col items-center gap-7 px-2 pb-3">
          ...
          <p className="min-h-4 text-center text-[length:var(--app-font-size-ui,12px)] leading-snug text-muted-foreground">
            {status ?? ""}
          </p>
        </div>
      </DialogPopup>
