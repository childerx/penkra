...
export const AppListRowShared = forwardRef<HTMLButtonElement, AppListRowSharedProps>(
  function AppListRowShared(
    ...
    return (
      <button
        className={cn(
          "flex min-h-[29px] w-full cursor-pointer items-center gap-2 rounded-md border-0 bg-transparent px-2.5 font-sans text-[length:var(--app-font-size-ui-lg,13px)] text-[var(--color-text-foreground-secondary)] outline-none transition-colors hover:bg-[var(--color-background-button-secondary-hover)] hover:text-[var(--color-text-foreground)] focus-visible:ring-1 focus-visible:ring-[var(--color-border-focus)] disabled:cursor-not-allowed disabled:text-[var(--color-text-foreground-tertiary)]",
          selected &&
      ...
      >
        ...
        {shortcut ? (
          <kbd className="font-sans text-[length:var(--app-font-size-ui-sm,11px)] text-[var(--color-text-foreground-tertiary)]">
            {shortcut}
          </kbd>
        ) : (
