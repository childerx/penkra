...
describe("App operation schema boundary", () => {
  it("compiles once-ready validators for valid input and output", () => {
    ...
    expect(() =>
      assertOperationValue({ title: "Fix redirect" }, validators.input, "input"),
    ).not.toThrow();
    expect(() =>
      assertOperationValue({ id: "PEN-184" }, validators.output, "output"),
    ).not.toThrow();
    expect(() => assertOperationValue({ title: "" }, validators.input, "input")).toThrow(
      "declared schema",
    );
  });
  ...
  it("bounds invocation data before validation", () => {
    ...
    for (let depth = 0; depth <= APP_OPERATION_VALUE_MAX_DEPTH; depth += 1)
      nested = { value: nested };
    expect(() => assertOperationValue(nested, validators.input, "input")).toThrow(
      "maximum JSON depth",
    );

    const circular: Record<string, unknown> = { title: "Fix redirect" };
    circular.self = circular;
    expect(() => assertOperationValue(circular, validators.input, "input")).toThrow(
      "circular references",
    );
  });
});
