...
describe("development App sideload bootstrap", () => {
  ...
  it("updates changed sideload bytes through the runtime-safe swap", async () => {
    ...
    await expect(bootstrapDevelopmentSideload({
      packages: { ingestDirectory: vi.fn(async () => verified) },
      installations: {
        snapshot: () => ({ packagesByAppId: { "com.example.canvas": { source: "sideload", sha256: "b".repeat(64) } } }),
        updateSideloadForSpaces,
      },
    } as never, "/work/canvas")).resolves.toBe("updated");
    expect(updateSideloadForSpaces).toHaveBeenCalledWith(verified);
  });
  ...
  it("does not override a registry installation", async () => {
    await expect(bootstrapDevelopmentSideload({
      packages: { ingestDirectory: vi.fn(async () => verified) },
      installations: {
        snapshot: () => ({ packagesByAppId: { "com.example.canvas": { source: "registry", sha256: "b".repeat(64) } } }),
      },
    } as never, "/work/canvas")).rejects.toThrow("already installed from the registry");
  });
});
