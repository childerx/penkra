...
function fixture() {
  ...
  FS.writeFileSync(Path.join(packagePath, "INSTRUCTIONS.md"), "Confirm the destination project first.\n");
  ...
  state = setSpaceAppEnabled(state, { appId: "com.acme.linear", spaceId: "personal", enabled: true });
...
describe("App operation catalog", () => {
  it("lists and renders help only for Apps enabled in the selected Space", async () => {
    ...
    expect(catalog.list("personal")).toEqual([expect.objectContaining({
      slug: "linear",
      operations: [{ key: "issues.create", summary: "Create an issue." }],
    })]);
  ...
  it("rejects symlinked package instructions", async () => {
    ...
    const outside = Path.join(Path.dirname(packagePath), `${Path.basename(packagePath)}-outside.md`);
    roots.push(outside);
    FS.writeFileSync(outside, "outside\n");
    FS.rmSync(Path.join(packagePath, "INSTRUCTIONS.md"));
    FS.symlinkSync(outside, Path.join(packagePath, "INSTRUCTIONS.md"));
    await expect(new AppOperationCatalog(() => state).help({ spaceId: "personal", slug: "linear" }))
