...
describe("App tab IPC boundary", () => {
  it("parses lifecycle requests without coercion", () => {
    expect(parseOpenAppTabRequest({ appId: "app", spaceId: "space", threadId: "thread", route: "/" })).toEqual({ appId: "app", spaceId: "space", threadId: "thread", route: "/" });
    ...
    expect(parseSetAppTabVisibleRequest({ tabId: "tab", visible: true })).toEqual({ tabId: "tab", visible: true });
    expect(parseSetAppTabBoundsRequest({ tabId: "tab", bounds: { x: 1, y: 2, width: 3, height: 4 } })).toEqual({ tabId: "tab", bounds: { x: 1, y: 2, width: 3, height: 4 } });
  });
  ...
  it("rejects missing identities and non-finite geometry", () => {
    ...
    expect(() => parseSetAppTabBoundsRequest({ tabId: "tab", bounds: { x: 0, y: 0, width: Infinity, height: 4 } })).toThrow();
  });
});
