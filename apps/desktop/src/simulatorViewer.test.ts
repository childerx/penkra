...
describe("Simulator viewer", () => {
  ...
  it("routes normalized pointer and keyboard input to the owned session", async () => {
    ...
    await invokeSimulatorViewerInput(target, owner, {
      method: "swipe",
      value: { from: { x: 0.1, y: 0.9 }, to: { x: 0.9, y: 0.1 }, durationMs: 350 },
    });
    await invokeSimulatorViewerInput(target, owner, { method: "type", value: "hello" });
  ...
  it("rejects invalid coordinates, durations, text, and methods", async () => {
    const target = manager();
    await expect(
      invokeSimulatorViewerInput(target, owner, { method: "tap", value: { x: -1, y: 0 } }),
    ).rejects.toMatchObject({ code: "INVALID_INPUT" });
    await expect(
      invokeSimulatorViewerInput(target, owner, {
        method: "swipe",
        value: { from: { x: 0, y: 0 }, to: { x: 1, y: 1 }, durationMs: Infinity },
      }),
    ).rejects.toMatchObject({ code: "INVALID_INPUT" });
    await expect(
      invokeSimulatorViewerInput(target, owner, { method: "type", value: "x".repeat(10_001) }),
    ).rejects.toMatchObject({ code: "INVALID_INPUT" });
    await expect(
      invokeSimulatorViewerInput(target, owner, { method: "press", value: "home" }),
    ).rejects.toMatchObject({ code: "INVALID_INPUT" });
  });
});
