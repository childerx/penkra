# Penkra Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Penkra for lifecycle, cancellation, queues, errors, and application policy.
...
| Commit              | Summary                                       |                                         Size |
| ------------------- | --------------------------------------------- | -------------------------------------------: |
| `0e5f6af9c`         | Refactor Penkra orchestration and web UI flow |                  387 files, +43,323 / -9,299 |
...
AcpSessionRuntime
  ...
  └─ Penkra: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
| Workstream                              | Why it was needed                                                                   | Main change                                                             | Result to verify                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `P2-ACP-01` ACP foundation              | Penkra maintained a custom wire stack beside the official SDK                       | Official SDK owns all production ACP wire behavior; custom wire deleted | Grok, Droid, and Cursor have one wire implementation and no fallback   |
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Penkra's application
lifecycle.
...
| Provider process supervision                  | Penkra/Effect          |
| Queue and resource limits                     | Penkra/Effect          |
| Session/product policy                        | Penkra/Effect          |
| Normalized Penkra events                      | Penkra/Effect          |
| Error translation into Penkra domain errors   | Thin local adapter     |
...
3. Provider adapters should depend on a stable Penkra runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Penkra. A
...
SQLite persistence, or the full Penkra app.
...
```text
Review the Penkra branch `codex/audit-code-quality` as a senior architecture, correctness,
