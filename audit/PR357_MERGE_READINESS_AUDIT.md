# Penkra PR #357 — Final code-quality, ACP, and provider audit

**Audit date:** 2026-07-15
**PR:** [#357 — Audit and harden code quality across desktop and server paths](https://github.com/Emanuele-web04/penkra/pull/357)
...
- after all tests, direct children of the Penkra server totalled about 1.84 GiB RSS, dominated by
  OpenCode;
...
as the sole protocol/wire authority and keep Effect above it as Penkra's lifecycle and application
...
- `AcpSessionRuntime.ts` constructs `OfficialAcp.client({ name: "penkra" })`, registers client
...
client/agent builders, handlers, streams, validation, and connection lifecycle that Penkra would
...
| Protocol upgrades              | Penkra had to regenerate and reconcile behavior | Follow the official SDK and negotiate protocol capabilities         |
| Process lifecycle              | Effect                                          | Effect, unchanged                                                   |
| Backpressure and queue budgets | Effect/custom transport                         | Penkra-owned bounded bridge around the official SDK                 |
| Canonical provider events      | Penkra adapters                                 | Penkra adapters, unchanged                                          |
...
- Closing the browser reduced the retained process from roughly 651 MiB to 135 MiB; stopping Penkra
...
- [ ] Do not recreate official SDK types in a new Penkra package.
