...
process supervision, teardown, and Penkra lifecycle policy. The official
...
2. `effect` for Penkra runtime and lifecycle ownership.
3. Two small local modules beside the ACP runtime:
   - `AcpErrors.ts`, containing only the Effect-native errors Penkra still consumes;
...
- `AcpErrors.ts` owns only Penkra's three Effect-native runtime errors, and `AcpExtensions.ts` owns
...
1. A minimal Effect codec for the parts of `SessionConfigOption` Penkra reads.
...
3. Delete the redundant Effect-Schema decode of `session/update`; the official SDK already validates
   it before invoking Penkra's handler.
...
7. Normalize SDK `null` values at consumption sites where Penkra uses `undefined`; do not introduce
...
- Effect still owns Penkra runtime/lifecycle policy;
